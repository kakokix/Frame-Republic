...
class App(tk.Tk):
    ...
    def _gpu_optim_nvidia(self):
        """Applique les reglages NVIDIA gaming optimaux via registre."""
        try:
            # ── Global Profile (applique a tous les jeux) ──
            ...
            # (3D Settings > Manage 3D > Power management)
            reg_set(wr.HKEY_LOCAL_MACHINE,
                    r"SYSTEM\CurrentControlSet\Services\nvlddmkm\Global\NVTweak",
                    "PowerMizerEnable", 1)
            reg_set(wr.HKEY_LOCAL_MACHINE,
                    r"SYSTEM\CurrentControlSet\Services\nvlddmkm\Global\NVTweak",
                    "PerfLevelSrc", 0x2222)  # Prefer max perf
            reg_set(wr.HKEY_LOCAL_MACHINE,
                    r"SYSTEM\CurrentControlSet\Services\nvlddmkm\Global\NVTweak",
                    "PowerMizerLevel",   1)
            reg_set(wr.HKEY_LOCAL_MACHINE,
            ...
            # 2. Desactiver VSync globalement (V-Sync = Off)
            reg_set(wr.HKEY_LOCAL_MACHINE,
                    r"SOFTWARE\NVIDIA Corporation\Global\NVTweak",
                    "VSync", 0)

            # 3. Reduire le pre-rendered frames au minimum
            # (Max Frames Rendered Ahead = 1)
            reg_set(wr.HKEY_LOCAL_MACHINE,
                    r"SYSTEM\CurrentControlSet\Services\nvlddmkm\Global\NVTweak",
                    "PrerenderLimit", 1)

            # 4. Threaded optimization = On (multi-thread CPU)
            reg_set(wr.HKEY_LOCAL_MACHINE,
                    r"SYSTEM\CurrentControlSet\Services\nvlddmkm",
                    "DisableThreadedOptimization", 0)

            # ── Methode principale: NVIDIA Profile Inspector CLI ──
            # On utilise nvidia-smi + nvapi via PowerShell si disponible
            ps_cmd = (
                "try {"
                "  Get-Process nvidia-settings -EA SilentlyContinue | "
                "    Stop-Process -Force -EA SilentlyContinue;"
                "} catch {}"
            )
            ps(ps_cmd, t=5)

            # ── ShadowPlay / NVIDIA Overlay OFF (reduit latence) ──
            reg_set(wr.HKEY_CURRENT_USER,
                    r"Software\NVIDIA Corporation\Global\ShadowPlay\NVSPCAPS",
                    "DefaultPathW", "")

            # ── NVIDIA Telemetry OFF ──
            for svc in ["NvTelemetryContainer", "NvContainerLocalSystem"]:
                run("sc config " + svc + " start= demand 2>nul")

            # ── Reflex Low Latency (NVIDIA Reflex Ultra) ──
            # Cle uniquement appliquee aux jeux compatibles
            reg_set(wr.HKEY_LOCAL_MACHINE,
                    r"SOFTWARE\NVIDIA Corporation\Global\NGXCore",
                    "ReflexMode", 2)  # 2 = Ultra

            self._dlog("OK NVIDIA: VSync off, Max performance, Reflex Ultra, Pre-render=1")
            self._dlog("Redemarrez vos jeux pour appliquer les changements")

        except Exception as e:
            self._dlog("Erreur NVIDIA: " + str(e)[:60])
