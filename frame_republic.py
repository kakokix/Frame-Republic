...
# ================================================================
#  INSTANCE UNIQUE - empeche 2 lancements simultanes
# ================================================================

def _check_single_instance():
    """Si une instance tourne deja, on lui envoie un signal pour qu elle
    remonte sa fenetre, puis on quitte."""
    try:
        import ctypes
        from ctypes import wintypes

        # Creer un mutex nomme unique a l app
        MUTEX_NAME = "FrameRepublic_SingleInstance_Mutex_kakokix"
        ...
        if last_error == ERROR_ALREADY_EXISTS:
            # Instance deja lancee: chercher sa fenetre cachee dans le tray
            # et lui dire de se montrer
            user32 = ctypes.windll.user32

            # Trouver la fenetre tray FrameRepublic
            hwnd = user32.FindWindowW("FrameRepublicTray", "FrameRepublic")
            if hwnd:
                # Message custom WM_APP+2 = "show window"
                WM_APP = 0x8000
                WM_SHOW_MAIN = WM_APP + 2
                user32.PostMessageW(hwnd, WM_SHOW_MAIN, 0, 0)
            else:
                # Pas de tray - chercher la fenetre principale directement
                # Parcourir les fenetres top-level
                ...
                def enum_cb(hwnd, lparam):
                    ...
                    if "Frame Republic" in buf.value or "FrameRepublic" in buf.value:
                        found[0] = hwnd
                        return False
                    return True
...
# Verifier avant tout le reste
_instance_handle = _check_single_instance()
...
# ================================================================
#  BOUTON ARRONDI - vrais rounded rectangles via Canvas
# ================================================================
class RoundBtn(tk.Canvas):
    """Bouton avec coins vraiment arrondis (border-radius style site)."""
    ...
    def _draw(self, bg_color, fg_color, border_color):
        ...
        # Bordure si ghost (utilise des lignes et arcs)
        if border_color:
            # Top et bottom
            self.create_line(r, 0, w-r, 0, fill=border_color, width=1)
            self.create_line(r, h-1, w-r, h-1, fill=border_color, width=1)
            # Left et right
            self.create_line(0, r, 0, h-r, fill=border_color, width=1)
            self.create_line(w-1, r, w-1, h-r, fill=border_color, width=1)
            # Arcs (contour des coins)
            self.create_arc(0, 0, 2*r, 2*r, start=90, extent=90,
                             outline=border_color, style="arc", width=1)
            self.create_arc(w-2*r, 0, w, 2*r, start=0, extent=90,
                             outline=border_color, style="arc", width=1)
            self.create_arc(0, h-2*r, 2*r, h, start=180, extent=90,
                             outline=border_color, style="arc", width=1)
    ...
    def _on_release(self, e):
        ...
        if self._hovered:
            self._draw(self._bg_hover, self._fg_hover, self._border)
            if self._cmd:
                threading.Thread(target=self._cmd, daemon=True).start()
        else:
            self._draw(self._bg_normal, self._fg_normal, self._border)
...
def _show_auth_window():
    ...
    # Bouton valider - rouge plein arrondi (style site)
    btn_w = 300
    btn_h = 42
    btn_r = 12
    btn = _tk.Canvas(body, width=btn_w, height=btn_h,
                      bg=_CARD, highlightthickness=0, cursor="hand2")
    btn.pack(fill="x", pady=6, padx=8)

    def _draw_btn(bg_color, fg_color):
        btn.delete("all")
        # Rectangle arrondi: 2 rects + 4 arcs
        btn.create_rectangle(btn_r, 0, btn_w-btn_r, btn_h,
                              fill=bg_color, outline="")
        btn.create_rectangle(0, btn_r, btn_w, btn_h-btn_r,
                              fill=bg_color, outline="")
        btn.create_arc(0, 0, 2*btn_r, 2*btn_r,
                        start=90, extent=90, fill=bg_color, outline="")
        btn.create_arc(btn_w-2*btn_r, 0, btn_w, 2*btn_r,
                        start=0, extent=90, fill=bg_color, outline="")
        btn.create_arc(0, btn_h-2*btn_r, 2*btn_r, btn_h,
                        start=180, extent=90, fill=bg_color, outline="")
        btn.create_arc(btn_w-2*btn_r, btn_h-2*btn_r, btn_w, btn_h,
                        start=270, extent=90, fill=bg_color, outline="")
        btn.create_text(btn_w/2, btn_h/2, text="VALIDER LE CODE",
                         fill=fg_color, font=(_F, 10, "bold"))

    _draw_btn(_A, _WHITE)
    btn.bind("<Enter>", lambda e: _draw_btn("#ff5577", _WHITE))
    btn.bind("<Leave>", lambda e: _draw_btn(_A, _WHITE))
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # Coins arrondis Windows 11 (DWM API)
        try:
            # DWMWA_WINDOW_CORNER_PREFERENCE = 33 (Win 11 22000+)
            # DWMWCP_ROUND = 2 = coins completement arrondis
            hwnd = ctypes.windll.user32.GetParent(self.winfo_id())
            if not hwnd:
                hwnd = ctypes.windll.user32.FindWindowW(None, "Frame Republic")
    ...
    def _setup_autostart(self):
        """Ajoute le logiciel au demarrage Windows en mode arriere-plan."""
        try:
            import winreg as wr
            exe_path = _get_exe_path()
            # Si c est l exe, on le lance avec --tray pour demarrer minimise
            if _is_frozen():
                cmd = '"' + exe_path + '" --tray'
            else:
                cmd = '"' + sys.executable + '" "' + exe_path + '" --tray'

            key = wr.OpenKey(wr.HKEY_CURRENT_USER,
                r"Software\\Microsoft\\Windows\\CurrentVersion\\Run",
                0, wr.KEY_SET_VALUE)
            wr.SetValueEx(key, "FrameRepublic", 0, wr.REG_SZ, cmd)
            wr.CloseKey(key)
            return True
        except Exception:
            return False
    ...
    def _remove_autostart(self):
        """Retire le logiciel du demarrage Windows."""
        try:
            import winreg as wr
            key = wr.OpenKey(wr.HKEY_CURRENT_USER,
                r"Software\\Microsoft\\Windows\\CurrentVersion\\Run",
                0, wr.KEY_SET_VALUE)
            try:
                wr.DeleteValue(key, "FrameRepublic")
    ...
    def _is_autostart_enabled(self):
        """Verifie si le demarrage auto est active."""
        try:
            import winreg as wr
            key = wr.OpenKey(wr.HKEY_CURRENT_USER,
                r"Software\\Microsoft\\Windows\\CurrentVersion\\Run")
            try:
                v, _ = wr.QueryValueEx(key, "FrameRepublic")
                wr.CloseKey(key)
                return bool(v)
    ...
    def _do_autostart(self, state=True):
        """Toggle du demarrage automatique avec Windows."""
        if state:
        ...
        try:
            ...
            def wnd_proc(hwnd, msg, wparam, lparam):
                if msg == WM_TRAY:
                    ...
                    # Signal "afficher" envoye par une 2e instance
                    self.after(0, self._show_window)
                    return 0
                elif msg == WM_DESTROY:
                    return 0
    ...
    def _card_grid(self, parent, cards_data):
        ...
        for i, d in enumerate(cards_data):
            ...
            # Stocker les cartes pour le layout responsive
            if not hasattr(self, '_responsive_grids'):
                self._responsive_grids = {}
            if id(grid) not in self._responsive_grids:
                self._responsive_grids[id(grid)] = []
            self._responsive_grids[id(grid)].append(card)

        # Layout responsive: nb colonnes selon largeur
        def _relayout(*_):
            w = inner.winfo_width()
            if w < 100: return
            # Calcul: cartes ~260px ideales, avec padding 12px -> ~272px
            cols = max(1, min(6, w // 280))
            cards = self._responsive_grids.get(id(grid), [])
            # Reset config
            for c in range(6):
                try: grid.columnconfigure(c, weight=0, minsize=0)
                except: pass
            # Re-placer les cartes
        ...
        def _combined_config(e):
            cv.configure(scrollregion=cv.bbox("all"))
            _relayout()
        inner.bind("<Configure>", _combined_config)
        # Aussi bind sur le parent pour reagir au resize fenetre
        parent.bind("<Configure>", lambda e: grid.after(50, _relayout))
        grid.after(100, _relayout)

    def _mk_btn(self, parent, text, cmd, color=A, padx=18, pady=8, primary=False):
        """Cree un bouton avec vrais coins arrondis (style site)."""
        return RoundBtn(parent, text, command=cmd,
                        primary=primary, color=color,
                        padx=padx, height=34, radius=10)
    ...
    def _build_dashboard(self):
        ...
        self._ao_btn = RoundBtn(ao_btns, text="OPTIMISER MAINTENANT",
                                  command=self._ao_run,
                                  primary=True, padx=24, height=40, radius=12)
    ...
    def _build_page_confort(self):
        tabs = [
            ("Confort", [
        dict(id="autostart",    title="Demarrer avec Windows",  desc1="Lance Frame Republic en arriere-plan au demarrage.",   desc2="Optimisations actives en permanence.",        fn=self._do_autostart,      initial=False),
...
if __name__ == "__main__":
    ...
    _show_auth_window()
    _app = App()
    # Si lance avec --tray (demarrage Windows), cacher directement
    if "--tray" in sys.argv:
        _app.after(300, _app._quit)  # cache direct -> tray
    _app.mainloop()
